...
final class AddPopUpPresentationController: UIPresentationController {
    ...
    @objc func swipeToDissmis(_ sender: UIPanGestureRecognizer) {
        ...
        if sender.state == .ended {
            UIView.animate(withDuration: 0.2, animations: { presentedView.transform = .identity })
        }
        
        if yTransition > 200 && sender.state == .ended {
            dismiss()
        }
    }
}
